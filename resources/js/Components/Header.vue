<template>
    <header class="flex flex-row mt-2 justify-between bg-gray-100 dark:bg-gray-900 sm:items-center sm:pt-0">
        <div class="m-4 flex flex-row justify-items-center space-x-2">
            ...
            <!-- Input search -->
            <div class="px-3 py-2 bg-white rounded-full flex flex-row space-x-1 justify-start items-center md:w-72">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
                    <path fill-rule="evenodd"
                ...
                <input v-model="search" type="text" placeholder="cari"
                    class="text-base border-none h-6 w-full rounded-full focus-within:outline-none">
            </div>
        </div>
                    ...
                    <button type="button" class="inline-flex max-w-xs items-center rounded-full text-sm dark:text-white">
                        ...
                        <div v-else class="flex items-center">
                            <img v-if="user.avatar" class="h-8 w-8 rounded-full" :src="`/storage/${user.avatar}`"
                                alt="avatar">
                            <span v-else
                                class="hidden md:flex md:shrink-0 justify-center items-center h-8 w-8 overflow-hidden rounded-full bg-gray-300 ">
